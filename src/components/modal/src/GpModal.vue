...
const scrollBarMaxHeight = computed(
    () => `calc(${unref(isFullscreen) ? '100vh' : '80vh'} - ${unref(modalHeaderHeight)}px${props.showAction ? ` - ${unref(modalActionHeight)}px` : null})`
);
...
<template>
    ...
    >
        ...
        <template v-if="showAction" #action>
            <NSpace justify="end">
                <NButton v-for="(action, index) in mergeActions" :key="index" v-bind="action" @click="() => handleClickAction(action)">
