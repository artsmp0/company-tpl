...
export const usePermissionStore = defineStore('permission', () => {
    ...
    const setRoutes = (menus: PermissionData | undefined) => {
        ...
        const generateRoutes = (list: MenuItemType[], parentPath?: string) => {
            ...
            list.forEach(item => {
                ...
                if (item.children && item.children.length > 0) {
                    const { flatMenuRoutes: childrenRoutes, menuRouter: childrenRouter } = generateRoutes(item.children, camel2kebab(item.path ?? ''));
        ...
        const otherPageRoutes = (menus.otherPage || []).map(item => generateRoute(item)).map(item => ({ ...item, meta: generateMeta(item.meta) }));
