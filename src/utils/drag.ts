...
const startDrag = function (eleBar: any, options: any) {
    ...
    for (const key in defaults) {
        if (typeof options[key] != 'undefined') params[key] = options[key];
    ...
    eleBar.addEventListener(objEventType.start, (event: any) => {
        // IE 拖拽可能拖不动的处理
        if (!window.WeakMap || typeof (document as any).msHidden != 'undefined') event.preventDefault();
        ...
        (event: any) => {
            if (store.isMoving) {
                ...
                if (params.edgeLock === true && bounding) {
                    ...
                    else if (distanceX > maxX) distanceX = maxX;

                    if (distanceY < minY) distanceY = minY;
                    else if (distanceY > maxY) distanceY = maxY;
                }
